...
export default function useUserData() {
  ...
  useEffect(() => {
    ...
    if (savedUserData) {
      ...
      if (parsedData && typeof parsedData.email === 'string') {
        setUserData(new UserData(parsedData.name, parsedData.surname, parsedData.email, parsedData.token, parsedData.cookingHours, parsedData.isLoggedIn));
      }
    }
  }, []);
  ...
  const preliminaryLogin = (email: string) => {
    setUserData(prev => {
      const updatedUser = new UserData(prev.name, "Tester", email, "", 0, true);
